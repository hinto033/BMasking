...
######
#Logistic Regression of each individual variable
#For Both CC and MLO
...
for (i in 6:nCols){
  # i<-7
  varT <- i
  modelDataCC <- trainSetCC[,c(4,varT)]
  ...
  modelDataMLO <- trainSetMLO[,c(4,varT)]
...
write.csv(regResults, file = paste(savePath,dateOfAnalysis,"regResults",i,".csv", sep=""),row.names=TRUE)
...
for (i in 1:10){
  ...
  plot(1:80, fittedInt, main=colnames(bestData[i]), pch=16,col=ifelse(fitted.results>0.5, "darkgreen", "red"))
  points( fittedScD, main=colnames(bestData[i]), pch=16,col=ifelse(fitted.results>0.5, "red", "darkgreen"))
  dev.off()
#   posInt<-fitted.results(1:40)
#   negInt<-fitted.results(41:80)
#   plot(1:80, posInt, col='o')
  fitted.results <- ifelse(fitted.results > 0.5,1,0)
  fittedInt <- fitted.results
  fittedInt[41:80] <- NA
  fittedScD <- fitted.results
...
for (i in 1:10){
  ...
  plot(1:100, fittedInt, main=colnames(bestData[i]), pch=16,col=ifelse(fitted.results>0.5, "darkgreen", "red"))
  points( fittedScD, main=colnames(bestData[i]), pch=16,col=ifelse(fitted.results>0.5, "red", "darkgreen"))
  dev.off()
  #   posInt<-fitted.results(1:40)
  #   negInt<-fitted.results(41:80)
...
'imgBMean', 'imgAMean', 'imgBMedian', 'imgAMedian','imgBSum','imgASum', 
...
fitted.results <- predict(smls,newdata=testPredictionsData,type='response')
fitted.results <- ifelse(fitted.results > 0.5,1,0)
misClasificError <- mean(fitted.results != testPredictionsData$Interval)
print(paste('Accuracy',1-misClasificError))
AccuracyCC<- 1-misClasificError
...
#   attach(trainData)
#   varT <- i
#   modelData <- allData[,c(4,varT)]
#   model <- glm(trainData$Interval~., family=binomial(logit), data=trainData)
#   summary(model)
#   
#   
#   
#   testPredictionsData <- testData
...
#####
#Analysis After splitting the MLO and CC



#####

# Analysis after splitting ensuring images from same woman are not placed into test and train set.
