function [cutoffs] = calcAFC(IDicomOrig, attenDisks,...
...
for p = 1:length(diameter) %For each Diam
    ...
    for k = 1:length(attenuation) %For each Thickness
        numCorrect = 0;
        for numTries = 1:15 % Number of times to do
            %% Create 1st patch
...
%             figure; imshow(imgWDisk,[])
            ...
            figure
            num = rand;
            if num < 0.5 %Place on the left
                ax(1) = subplot(1,2,1); imshow(imgWDisk,[]);
                ax(2) = subplot(1,2,2); imshow(im2Final,[]);
                Left = ax(1).Position(1); Right = ax(2).Position(1);
                [xSel,ySel] = ginput(1);  
                clickedAx = gca;
                ax = clickedAx;
                selection = ax.Position(1);
                if selection == Left
...
%                     disp('wrong1')
            ...
            elseif num >= 0.5 %Place on the right
                ax(2) = subplot(1,2,2); imshow(imgWDisk,[]);
                ax(1) = subplot(1,2,1); imshow(im2Final,[]);
                Left = ax(1).Position(1); Right = ax(2).Position(1);
                [xSel,ySel] = ginput(1);  
                clickedAx = gca;
                ax = clickedAx;
