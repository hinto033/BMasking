...
dateOfAnalysis = "5.8.17_SimilarStat_"
savePath = "W:\\Breast Studies\\Masking\\AnalysisImages\\"

# Similar By Statistics
wdInt <- "W:\\Breast Studies\\Masking\\BJH_MaskingMaps\\4.19.17_GenStats_SimilarStat\\Interval\\Comp\\"
wdScreen <- "W:\\Breast Studies\\Masking\\BJH_MaskingMaps\\4.19.17_GenStats_SimilarStat\\ScreenDetected\\Comp\\"
...
keep <- c('Interval', 'density', 'SXA..Breast.Density.', 'bmi', 'X_menopause_', 'X_firstdeg_', 'biop_hist', 
          "mediumMean", "mediumMedian", "mediumStDev", "mediumSum", "mediumEntropy", "mediumKurtosis" ,"mediumSkewness",
          "mediumPctile10", "mediumPctile25", "mediumPctile75", "mediumPctile90", "mediumPctBelowPnt5",
          "mediumPctBelow1", "mediumPctBelow1Pnt25", "mediumPctBelow1Pnt5",  "mediumPctBelow2", "mediumPctBelow3",
          "mediumPctBelow4", "mediumPctBelow5", "mediumPctBelow7", "mediumPctBelow10", "mediumPctBelow12", "mediumPctBelow14",    
           "mediumPctBelow16", "mediumPctBelow18", "mediumPctBelow20", "mediumGLCMContrast", "mediumGLCMCorr",      
           "mediumGLCMEnergy", "mediumGLCMHomog", "imgBMean", "imgBMedian", "imgBSum", "imgBPctile10",
           "imgBPctile25", "imgBPctile75", "imgBPctile90", "imgAMean", "imgAMedian", "imgASum",
           "imgAPctile10", "imgAPctile25", "imgAPctile75","imgAPctile90")


#CC Case
setwd(savePath)
fileConn<-file("kFolds_CC_StepwiseTrain.txt")
...
for(j in 1:10){
  ...
  predData <- na.omit(trainSetCC[keep])
  model <- glm(predData$Interval~., family=binomial(logit), data=predData)
  modelStep <- step(model)
  summary(modelStep)
  out <- capture.output(summary(modelStep))
  cat(out,file="kFolds_CC_StepwiseTrain.txt",sep="\n",append=TRUE)
  
  predData <- na.omit(testSetCC[keep])
  fitted.results <- predict(modelStep,newdata=predData,type='response')
  fitted.results <- ifelse(fitted.results > 0.5,1,0)
  misClasificError <- mean(fitted.results != predData$Interval)
  print(paste('Accuracy',1-misClasificError))
  AccuracyCC[j]<- 1-misClasificError
  p <- predict(modelStep, newdata=predData, type="response")
  pr <- prediction(p, predData$Interval)
...
setwd(savePath)
fileConn<-file("kFolds_MLO_StepwiseTrain.txt")
...
for(j in 1:10){
  ...
  predData <- na.omit(trainSetMLO[keep])
  model <- glm(predData$Interval~., family=binomial(logit), data=predData)
  modelStep <- step(model)
  summary(modelStep)
  out <- capture.output(summary(modelStep))
  cat(out,file="kFolds_MLO_StepwiseTrain.txt",sep="\n",append=TRUE)
  
  predData <- na.omit(testSetMLO[keep])
  fitted.results <- predict(modelStep,newdata=predData,type='response')
  fitted.results <- ifelse(fitted.results > 0.5,1,0)
  misClasificError <- mean(fitted.results != predData$Interval)
  print(paste('Accuracy',1-misClasificError))
